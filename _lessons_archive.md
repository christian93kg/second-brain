...
> [!info] Verbatim archive
> Full lesson entries live here; `_lessons.md` holds the distilled digest read at session start. Headings are identical between the two files, so `[[_lessons_archive#<heading>]]` anchors match the digest's. Append-only; never delete — the history is the value.

# Lessons — full entries

Each lesson is one heading (the pattern), with two short blocks under it:

- **Why:** the reason the owner gave or the incident that prompted it.
- **How to apply:** when/where this kicks in.

Lint moves entries here from `_lessons.md` once they're older than ~30 days, leaving a condensed digest behind. Inherited starter lessons ship here in full from day one.
...
**Why:** `obsidian create overwrite path=... content="$content"` with a large content argument (observed at ~30 KB and up) cold-boots a fresh Electron instance and never applies the write — file mtime unchanged, no numbered duplicate, sometimes exit 124 / sometimes exit 0, both lying. Reads, `append`, `property:set`, and *small* `create overwrite` all hand off over the CLI socket fine; large argv content does not, and the cold-boot fallback doesn't write. `append` can't cover the gap either — it can't do a mid-file edit or a below-delimiter `GAPS.md` regen.

**How to apply:** the CLI has an `eval code="<js>"` command that runs JS **in-app** with the full `app.vault` API **and** `require('fs')`. Two patterns, both keep large content off argv:

1. **Targeted edit of a big file** (read-modify-write in-app — only tiny JS crosses the socket):
   ```
   obsidian eval code="(async()=>{const f=app.vault.getAbstractFileByPath('<folder>/<file>.md');let c=await app.vault.read(f);c=c.split('[[bare_id]]').join('[[parent#bare_id]]');await app.vault.modify(f,c);return 'ok';})()"
   ```
2. **Large body write** (e.g. a `GAPS.md` regen — land the body on disk first via the `Write` tool, read it inside the app via `fs`):
   ```
   # Write tool → /tmp/claude/body.md, then:
   obsidian eval code="(async()=>{const fs=require('fs');const b=fs.readFileSync('/tmp/claude/body.md','utf8');const g=app.vault.getAbstractFileByPath('GAPS.md');await app.vault.modify(g,b);return b.length;})()"
   ```

**eval gotchas:** the param is `code=` (not `content=`); the snippet is a **top-level expression**, so no bare `return` — wrap statements in an `(async()=>{ ... })()` IIFE. `app.vault.modify` writes through Obsidian's own API, so Sync propagates it correctly — `eval` *is* the CLI, not a filesystem bypass. For a below-delimiter regen, read the live file in-app, split on the marker, keep the prefix, concat the new body, then `modify` — the owner's manual prefix never ships over argv. Build the JS via the `Write` tool into `/tmp/claude/<f>.js` then `js=$(cat /tmp/claude/<f>.js); obsidian eval code="$js"` to dodge the shell-quoting hazards covered by the heredoc/tempfile lessons above.
